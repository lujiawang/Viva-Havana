...
namespace OceanToolkit
{
    ...
    public class OceanShaderEditor : ShaderGUI
    {
        ...
        protected static string ClipOff = "OT_CLIP_OFF";
        protected static string ClipOn = "OT_CLIP_ON";
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            ...
            if (System.Array.IndexOf(m.shaderKeywords, ReflOff) != -1)
            {
                refl = Refl.Off;
            }
            else if (System.Array.IndexOf(m.shaderKeywords, ReflSkyOnly) != -1)
            {
                refl = Refl.SkyOnly;
            }
            else if (System.Array.IndexOf(m.shaderKeywords, ReflSSR) != -1)
            {
            ...
            if (System.Array.IndexOf(m.shaderKeywords, RefrOff) != -1)
            {
                refr = Refr.Off;
            }
            else if (System.Array.IndexOf(m.shaderKeywords, RefrColor) != -1)
            {
                refr = Refr.Color;
            }
            else if (System.Array.IndexOf(m.shaderKeywords, RefrNormalOffset) != -1)
            {
            ...
            if (System.Array.IndexOf(m.shaderKeywords, ClipOff) != -1)
            {
                clip = false;
            }
            if (System.Array.IndexOf(m.shaderKeywords, ClipOn) != -1)
            {
                clip = true;
            }
            ...
            clip = EditorGUILayout.Toggle("Clip Bounds", clip);
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (clip == false)
                {
                    m.EnableKeyword(ClipOff);
                    m.DisableKeyword(ClipOn);
                }
                else if (clip == true)
                {
                    m.DisableKeyword(ClipOff);
                    m.EnableKeyword(ClipOn);
                }
