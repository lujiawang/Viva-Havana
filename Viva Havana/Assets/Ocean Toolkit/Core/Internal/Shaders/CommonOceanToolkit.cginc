...
inline float3 sampleSky(float3 dir)
{
	...
	// From inside UnityGlossyEnvironmentSetup(half Smoothness, half3 worldViewDir, half3 Normal, half3 fresnel0)
	Unity_GlossyEnvironmentData g;
	g.roughness /* perceptualRoughness */ = SmoothnessToPerceptualRoughness(1.0);
	g.reflUVW = dir;

	//UnityGI_IndirectSpecular(UnityGIInput data, half occlusion, Unity_GlossyEnvironmentData glossIn)
    return UnityGI_IndirectSpecular(data, 1.0, g);
}
