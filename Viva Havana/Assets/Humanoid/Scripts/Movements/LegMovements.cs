...
namespace Passer {
    ...
    public class LegMovements : Movements {
        ...
        public static void Update(FootTarget footTarget) {
            ...
            if (footTarget.humanoid.targetsRig.runtimeAnimatorController != null && footTarget.foot.target.confidence.position == 0) {
        ...
        private void FullInverseKinematics(FootTarget footTarget) {
            ...
            if (!Application.isPlaying) {
                float footTargetDistance = Vector3.Distance(footTarget.foot.bone.transform.position, footTarget.foot.target.transform.position);
                if (footTargetDistance < 0.001F) {
                    footTarget.foot.bone.transform.position = footTarget.foot.target.transform.position;
                }
            }
        }
        ...
        private Vector3 NaturalFootPosition(FootTarget footTarget) {
            if (footTarget.ground == null)
                ...
                //Debug.Log(footTarget + " " + footTarget.groundDistance);
                //Debug.DrawRay(footTarget.foot.target.transform.position, footTarget.groundDistance * footTarget.humanoid.up);
