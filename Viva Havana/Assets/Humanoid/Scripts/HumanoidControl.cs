...
namespace Passer {
    ...
    public class HumanoidControl : MonoBehaviour {
        ...
        public void SetTrackingHeightToAvatar() {
            ...
            float deltaY = avatarNeckHeight - localNeckHeight;
        ...
        //    if (headTarget != null)
        //        DestroyImmediate(headTarget.gameObject, true);
        //    if (leftHandTarget != null)
        //        DestroyImmediate(leftHandTarget.gameObject, true);
        //    if (rightHandTarget != null)
        //        DestroyImmediate(rightHandTarget.gameObject, true);
        //    if (hipsTarget != null)
        //        DestroyImmediate(hipsTarget.gameObject, true);
        //    if (leftFootTarget != null)
        //        DestroyImmediate(leftFootTarget.gameObject, true);
        //    if (rightFootTarget != null)
        //        DestroyImmediate(rightFootTarget.gameObject, true);

        //    if (targetsRig != null)
        //        DestroyImmediate(targetsRig.gameObject, true);
        //}
        //#endregion
        ...
        public virtual void MoveForward(float z) {
            ...
            //targetVelocity += Vector3.forward * z;
        }
        ...
        public virtual void Jump(float velocity) {
            ...
            float y = targetVelocity.y + velocity;
        ...
        private void CalculateMovement() {
            ...
            //humanoidRigidbody.MovePosition(transform.position + translationVector * Time.deltaTime);
            if (targetVelocity.y != 0) {
                if (ground == null)
                    targetVelocity.y = 0;
                transform.position += targetVelocity.y * Vector3.up * Time.deltaTime;
            }
        }
        ...
        private void CheckBodyCollider(GameObject collidersObject) {
            ...
            spineColliderObject.tag = this.gameObject.tag;
            spineColliderObject.layer = this.gameObject.layer;
            ...
            leftColliderObject.tag = this.gameObject.tag;
            leftColliderObject.layer = this.gameObject.layer;
            ...
            rightColliderObject.tag = this.gameObject.tag;
            rightColliderObject.layer = this.gameObject.layer;
        ...
        private void CheckGrounded() {
            if (!leftHandTarget.GrabbedStaticObject() && !rightHandTarget.GrabbedStaticObject()) {
                ...
                Vector3 fallDirection = Vector3.Project(gravitationalVelocity, -this.up);
                float maxDistance = stepOffset + fallDirection.magnitude * Time.fixedDeltaTime;
                float distance = GetDistanceToGroundAt(footBase, maxDistance, out ground, out groundNormal);
                if (distance > 0.01F) {
        ...
        private void CheckBodyPull() {
            ...
            //this.transform.Translate(pullVector, Space.World);

            var collisionAngle = Vector3.Angle(pullVector, hitNormal);

            //if (collided) Debug.DrawRay(hitPoint, hitNormal, Color.red, .5f);

            if (!collided || (collided && collisionAngle <= 90f)) {
                transform.Translate(pullVector, Space.World);
            }
            //else if (collided && collisionAngle > 90f) {
            //    var modifiedPull = Vector3.ProjectOnPlane(pullVector, hitNormal);
            //    transform.Translate(modifiedPull, Space.World);
            //}
        }
        #endregion
        ...
        private void PostAnimationCorrection() {
            ...
            if (animatorEnabled && animatorController != null) {
                //avatarRig.transform.position = targetsRig.transform.position;
                //avatarRig.transform.rotation = targetsRig.transform.rotation;
                //if (targetsRig.transform.parent == avatarRig.transform) {
                //    targetsRig.transform.localPosition = Vector3.zero;
                //    targetsRig.transform.localRotation = Quaternion.identity;
                //}
