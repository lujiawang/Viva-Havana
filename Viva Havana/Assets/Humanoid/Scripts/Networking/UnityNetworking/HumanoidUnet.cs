...
#else
    public class HumanoidUnet : MonoBehaviour {
#endif
#if hNW_UNET
        ...
        protected void UpdateRemotePlayers() {
            foreach (HumanoidControl humanoid in humanoids) {
                if (humanoid.isRemote) {
                    Extrapolate(humanoid);
                }
            }
        }

        protected void Extrapolate(HumanoidControl humanoid) {
            Vector3 translation = humanoid.velocity * Time.deltaTime;
            humanoid.transform.Translate(translation, Space.World);
        }
        ...
        private bool serverPhysics;

        [Command] // @ server
        private void CmdServerStartNoHumanoid(int nwId) {
            serverNwId = nwId;
        ...
        [Command] // @ server
        private void CmdServerStartHumanoid(
            int nwId,
            int humanoidId,
            string name,
            ...
            Debug.Log("CmdServerStartHumanoid");
        ...
        public void NewClientStarted() {
            if (serverAvatarPrefabName == null || serverAvatarPrefabName == "")
                return;

            if (debug <= HumanoidNetworking.Debug.Info)
                Debug.Log(serverNwId + ": Forward StartHumanoid " + serverHumanoidId);

            RpcClientStartHumanoid(
                serverNwId,
                serverHumanoidId, 
                serverName, 
                serverAvatarPrefabName, 
                serverPosition, serverRotation,
                serverPhysics);
        }
        ...
        [Command] // @ server
        public void CmdGrab(int nwId, int humanoidId, GameObject obj, bool leftHanded, bool rangeCheck) {
            if (debug <= HumanoidNetworking.Debug.Info)
                Debug.Log("Server: CmdGrab " + obj);

            RpcClientGrab(nwId, humanoidId, obj, leftHanded, rangeCheck);
        }
