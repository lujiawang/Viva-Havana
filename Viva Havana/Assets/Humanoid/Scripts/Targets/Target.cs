...
namespace Passer {
    ...
    public abstract class HumanoidTarget : Target {
        ...
        public static void GetDefaultBone(Animator rig, ref Transform boneTransform, HumanBodyBones boneID, params string[] boneNames) {
            ...
            GetDefaultBone(rig, ref boneTransform, boneNames);
            //Transform hipsTransform = rig.GetBoneTransform(HumanBodyBones.Hips);

            //for (int i = 0; i < boneNames.Length; i++) {
            //    if (boneTransform != null)
            //        return;
                
            //    boneTransform = hipsTransform.FindDeepChild(boneNames[i]);
            //}
        }
        ...
        //    for (int j = 0; j < colliders.Length; j++) {
        //        Rigidbody colliderRigidbody = colliders[j].GetComponentInParent<Rigidbody>();
